...
type FancyButtonProps = {
  text: string;
  textAlt?: string; // texto que aparece en el "state-2" (cuando hace focus)
  onClick?: () => void;
  disabled?: boolean;
  fullWidth?: boolean;
};

function renderChars(text: string, stateClass: string) {
  const chars = Array.from(text);
  let i = 0;

  return (
    <span className={`${styles.char} ${stateClass}`}>
      {chars.map((ch, idx) => {
        if (ch === " ") {
          return (
            <span key={`sp-${idx}`} className={styles.space} aria-hidden="true">
              &nbsp;
            </span>
          );
        }
...
export default function FancyButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      style={fullWidth ? ({ width: "100%" } as React.CSSProperties) : undefined}
    >
  ...
  {renderChars(text, styles.state1)}
  {renderChars(alt, styles.state2)}
</div>
